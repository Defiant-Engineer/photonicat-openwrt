...
static void config_load(void)
{
	...
	if (uci_load(u, "photonicat", &p) == UCI_OK) {
		...
		if (s) {
			...
			if (val) cfg.backlight = atoi(val);
			val = uci_lookup_option_string(u, s, "refresh");
			if (val) cfg.refresh = atoi(val);
			val = uci_lookup_option_string(u, s, "theme");
			if (val) strncpy(cfg.theme, val, sizeof(cfg.theme)-1);
			val = uci_lookup_option_string(u, s, "font_scale");
			if (val) cfg.font_scale = strtof(val, NULL);
			...
			if (o && o->type == UCI_TYPE_LIST) {
				uci_foreach_element(&o->v.list, e) {
					if (cfg.num_pages < MAX_PAGES) {
						strncpy(cfg.pages[cfg.num_pages++], e->name, 15);
					}
				}
			}
		}
...
static void status_cb(struct ubus_request *req, int type, struct blob_attr *msg)
{
	...
	blobmsg_parse(status_policy, 8, tb, blobmsg_data(msg), blobmsg_data_len(msg));

	if (tb[0]) {
		blobmsg_parse(bat_policy, 4, bat, blobmsg_data(tb[0]), blobmsg_data_len(tb[0]));
		if (bat[0]) mdata.battery_pct = blobmsg_get_u32(bat[0]);
		if (bat[1]) mdata.battery_mv = blobmsg_get_u32(bat[1]);
		if (bat[2]) mdata.battery_ma = blobmsg_get_i32(bat[2]);
	}
	if (tb[1]) {
		blobmsg_parse(chg_policy, 2, chg, blobmsg_data(tb[1]), blobmsg_data_len(tb[1]));
		if (chg[0]) mdata.charger_online = blobmsg_get_u32(chg[0]);
	}
	if (tb[2]) mdata.board_temp = blobmsg_get_i32(tb[2]);
	if (tb[3]) mdata.fan_rpm = blobmsg_get_u32(tb[3]);
	if (tb[4]) mdata.fan_level = blobmsg_get_u32(tb[4]);
	mdata.valid = true;
}

static void update_mcu_data(void)
{
	if (!ubus_ctx) ubus_ctx = ubus_connect(NULL);
	if (!ubus_ctx) return;

	if (ubus_lookup_id(ubus_ctx, "photonicat", &photonicat_obj) == UBUS_STATUS_OK) {
		ubus_invoke(ubus_ctx, photonicat_obj, "status", NULL, status_cb, NULL, 500);
	}
}
...
static void draw_char(int x, int y, char ch, uint16_t fg, uint16_t bg, float s) {
    ...
    for (int col = 0; col < 6; col++) {
        ...
        for (int row = 0; row < 8; row++) {
            ...
            if (c != bg || bg != 0) fb_rect(x + col * s, y + row * s, s, s, c);
        }
    }
}

static int draw_str(int x, int y, const char *str, uint16_t fg, uint16_t bg, float s) {
    while (*str) {
...
static void draw_str_c(int y, const char *str, uint16_t fg, uint16_t bg, float s) {
...
static void spi_write(const uint8_t *data, size_t len) {
    struct spi_ioc_transfer tr = { .tx_buf = (unsigned long)data, .len = len, .speed_hz = SPI_SPEED_HZ, .bits_per_word = 8 };
    ioctl(spi_fd, SPI_IOC_MESSAGE(1), &tr);
}

static int gpio_request_output(int bank, int offset, const char *name, int initial) {
    char path[64]; snprintf(path, sizeof(path), "/dev/gpiochip%d", bank);
    int fd = open(path, O_RDWR); if (fd < 0) return -1;
    struct gpio_v2_line_request req = { .offsets[0] = offset, .num_lines = 1, .config.flags = GPIO_V2_LINE_FLAG_OUTPUT };
    strncpy(req.consumer, name, sizeof(req.consumer)-1);
    if (initial) { req.config.num_attrs = 1; req.config.attrs[0].attr.id = GPIO_V2_LINE_ATTR_ID_OUTPUT_VALUES; req.config.attrs[0].attr.values = 1; req.config.attrs[0].mask = 1; }
    if (ioctl(fd, GPIO_V2_GET_LINE_IOCTL, &req) < 0) { close(fd); return -1; }
    close(fd); return req.fd;
}
...
static void disp_init(void) {
    gpio_set(rst_line_fd, 0); usleep(50000); gpio_set(rst_line_fd, 1); usleep(50000);
    uint8_t cmd, data;
    cmd = CMD_SWRESET; gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); usleep(150000);
    cmd = CMD_SLPOUT;  gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); usleep(150000);
    cmd = CMD_COLMOD;  gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); data = 0x55; gpio_set(dc_line_fd, 1); spi_write(&data, 1);
    cmd = CMD_MADCTL;  gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); data = MADCTL_MX; gpio_set(dc_line_fd, 1); spi_write(&data, 1);
    cmd = CMD_INVOFF;  gpio_set(dc_line_fd, 0); spi_write(&cmd, 1);
    cmd = 0x13;        gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); /* NORON */
    cmd = 0x29;        gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); /* DISPON */
}
...
static void render_screen(void) {
    ...
    /* Battery */
    if (mdata.valid) {
        snprintf(tmp, sizeof(tmp), "BAT: %d%% %d.%02dV", mdata.battery_pct, mdata.battery_mv/1000, (mdata.battery_mv%1000)/10);
        draw_str_c(80, tmp, COL_GOOD, COL_BG, 1.5f);
        snprintf(tmp, sizeof(tmp), "TEMP: %dC FAN: %d", mdata.board_temp, mdata.fan_rpm);
    ...
    if (f) {
        float l1; fscanf(f, "%f", &l1); fclose(f);
        snprintf(tmp, sizeof(tmp), "LOAD: %.2f", l1);
        draw_str_c(140, tmp, COL_DIM, COL_BG, 1.5f);
    }

    /* Flush to SPI */
    uint8_t win[] = { 0, (uint8_t)COL_OFFSET, 0, (uint8_t)(DISP_W-1+COL_OFFSET) };
    uint8_t cmd = CMD_CASET; gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); gpio_set(dc_line_fd, 1); spi_write(win, 4);
    uint8_t rwin[] = { 0, 0, 1, (uint8_t)(DISP_H-1) };
    cmd = CMD_RASET; gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); gpio_set(dc_line_fd, 1); spi_write(rwin, 4);
    cmd = CMD_RAMWR; gpio_set(dc_line_fd, 0); spi_write(&cmd, 1);
    gpio_set(dc_line_fd, 1);
    for (int i = 0; i < sizeof(fb); i += SPI_CHUNK) {
        spi_write(fb + i, (sizeof(fb) - i > SPI_CHUNK) ? SPI_CHUNK : sizeof(fb) - i);
    }
}
...
int main(void) {
    ...
    while (running) {
        update_mcu_data();
        render_screen();
        for (int i = 0; i < cfg.refresh * 10 && running; i++) usleep(100000);
    }

    uint8_t cmd = 0x28; gpio_set(dc_line_fd, 0); spi_write(&cmd, 1); /* DISPOFF */
    gpio_set(bl_line_fd, 1); /* OFF */
    close(spi_fd); close(dc_line_fd); close(rst_line_fd); close(bl_line_fd);
    return 0;
}

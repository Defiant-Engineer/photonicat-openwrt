...
int mcu_send(mcu_ctx_t *ctx, uint16_t command, const uint8_t *extra_data,
	     uint16_t extra_len, bool need_ack)
{
	...
	/* Data length (LE): command(2) + extra_data + need_ack(1) = extra_len +
	 * 3 */
 ...
 * Returns 1 if a frame was parsed (and consumed from buffer), 0 if not enough
 * data.
 */
static int try_parse_frame(mcu_ctx_t *ctx, mcu_frame_t *frame)
{
	...
	while (i < ctx->rx_used) {
		...
		uint16_t expected_crc =
		    p[7 + data_len] | ((uint16_t)p[8 + data_len] << 8);
		uint16_t actual_crc = mcu_crc16(p + 1, 6 + data_len);
		if (expected_crc != actual_crc) {
			fprintf(
			    stderr,
			    "mcu: CRC mismatch: got 0x%04X, expected 0x%04X\n",
...
int mcu_parse_status(const uint8_t *data, uint16_t data_len,
		     mcu_status_t *status)
{
	if (data_len < 16)
		return -1;
	...
	if (data_len >= 20) {
		...
		status->battery_current_ma =
		    (int16_t)(data[18] | ((uint16_t)data[19] << 8));
...
int mcu_set_datetime(mcu_ctx_t *ctx, uint16_t year, uint8_t month, uint8_t day,
...
{
	uint8_t payload[3] = {shutdown_timeout, force_poweroff,
			      heartbeat_interval};
...
{
	switch (command) {
	case MCU_CMD_HEARTBEAT:
		return "HEARTBEAT";
	case MCU_CMD_HEARTBEAT_ACK:
		return "HEARTBEAT_ACK";
	case MCU_CMD_HW_VERSION_GET:
		return "HW_VERSION_GET";
	case MCU_CMD_HW_VERSION_GET_ACK:
		return "HW_VERSION_GET_ACK";
	case MCU_CMD_FW_VERSION_GET:
		return "FW_VERSION_GET";
	case MCU_CMD_FW_VERSION_GET_ACK:
		return "FW_VERSION_GET_ACK";
	case MCU_CMD_STATUS_REPORT:
		return "STATUS_REPORT";
	case MCU_CMD_STATUS_REPORT_ACK:
		return "STATUS_REPORT_ACK";
	case MCU_CMD_DATETIME_SYNC:
		return "DATETIME_SYNC";
	case MCU_CMD_DATETIME_SYNC_ACK:
		return "DATETIME_SYNC_ACK";
	case MCU_CMD_SCHEDULE_STARTUP_SET:
		return "SCHEDULE_STARTUP_SET";
	case MCU_CMD_SCHEDULE_STARTUP_SET_ACK:
		return "SCHEDULE_STARTUP_SET_ACK";
	case MCU_CMD_PMU_REQUEST_SHUTDOWN:
		return "PMU_REQUEST_SHUTDOWN";
	case MCU_CMD_PMU_REQUEST_SHUTDOWN_ACK:
		return "PMU_REQUEST_SHUTDOWN_ACK";
	case MCU_CMD_HOST_REQUEST_SHUTDOWN:
		return "HOST_REQUEST_SHUTDOWN";
	case MCU_CMD_HOST_REQUEST_SHUTDOWN_ACK:
		return "HOST_REQUEST_SHUTDOWN_ACK";
	case MCU_CMD_PMU_REQUEST_FACTORY_RESET:
		return "PMU_FACTORY_RESET";
	case MCU_CMD_PMU_REQUEST_FACTORY_RESET_ACK:
		return "PMU_FACTORY_RESET_ACK";
	case MCU_CMD_WATCHDOG_SET:
		return "WATCHDOG_SET";
	case MCU_CMD_WATCHDOG_SET_ACK:
		return "WATCHDOG_SET_ACK";
	case MCU_CMD_CHARGER_AUTO_START:
		return "CHARGER_AUTO_START";
	case MCU_CMD_CHARGER_AUTO_START_ACK:
		return "CHARGER_AUTO_START_ACK";
	case MCU_CMD_VOLTAGE_THRESHOLD_SET:
		return "VOLTAGE_THRESHOLD_SET";
	case MCU_CMD_VOLTAGE_THRESHOLD_SET_ACK:
		return "VOLTAGE_THRESHOLD_SET_ACK";
	case MCU_CMD_LED_SETUP:
		return "LED_SETUP";
	case MCU_CMD_LED_SETUP_ACK:
		return "LED_SETUP_ACK";
	case MCU_CMD_POWER_ON_EVENT_GET:
		return "POWER_ON_EVENT_GET";
	case MCU_CMD_POWER_ON_EVENT_GET_ACK:
		return "POWER_ON_EVENT_GET_ACK";
	case MCU_CMD_FAN_SET:
		return "FAN_SET";
	case MCU_CMD_FAN_SET_ACK:
		return "FAN_SET_ACK";
	case MCU_CMD_DEVICE_MOVEMENT:
		return "DEVICE_MOVEMENT";
	case MCU_CMD_DEVICE_MOVEMENT_ACK:
		return "DEVICE_MOVEMENT_ACK";
	default:
		return "UNKNOWN";
	}
}
...
{
	fprintf(stderr,
		"[%s] src=0x%02X dst=0x%02X frame#%u cmd=0x%04X (%s) "
		"extra=%u bytes ack=%d\n",
		(frame->src == MCU_ADDR_HOST) ? "TX" : "RX", frame->src,
		frame->dst, frame->frame_num, frame->command,
		mcu_cmd_name(frame->command), frame->extra_len,
		frame->need_ack);

		      ...
		      struct blob_attr *msg)
{
	...
	snprintf(rtc_str, sizeof(rtc_str), "%04u-%02u-%02u %02u:%02u:%02u",
		 last_status.rtc_year, last_status.rtc_month, last_status.rtc_day,
		 last_status.rtc_hour, last_status.rtc_min, last_status.rtc_sec);
...
static void mcu_read_cb(struct uloop_fd *u, unsigned int events)
{
	...
	while (mcu_recv(&mcu, &frame) == 1) {
		if (frame.command == MCU_CMD_STATUS_REPORT && frame.extra_len >= 16) {
			if (mcu_parse_status(frame.extra_data, frame.extra_len, &last_status) == 0) {
				status_valid = true;
			}
		}
